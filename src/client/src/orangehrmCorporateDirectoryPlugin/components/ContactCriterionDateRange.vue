...
<template>
    ...
    <oxd-grid-item v-if="operator && operator.id === 'between'" class="orangehrm-report-daterange --span-column-2">
        <oxd-input-field type="date" :rules="rules.valueX" :model-value="valueX" :placeholder="userDateFormat"
            :display-format="jsDateFormat" @update:model-value="$emit('update:valueX', $event)" />
        <oxd-text class="orangehrm-report-range-text" tag="p">to</oxd-text>
        <oxd-input-field type="date" :rules="rules.valueY" :model-value="valueY" :placeholder="userDateFormat"
            :display-format="jsDateFormat" @update:model-value="$emit('update:valueY', $event)" />
    </oxd-grid-item>
    <oxd-grid-item v-else-if="operator">
        <oxd-input-field type="date" :rules="rules.valueXOnly" :model-value="valueX" :placeholder="userDateFormat"
            :display-format="jsDateFormat" @update:model-value="$emit('update:valueX', $event)" />
    </oxd-grid-item>
</template>
...
export default {
    name: 'ContactCriterionDateRange',
    inheritAttrs: false,
    props: {
        operator: {
            type: Object,
